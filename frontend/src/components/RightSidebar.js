...
export const RightSidebar = ({ otherUsers }) => {
  ...
  return (
    <div className="md:block hidden md:w-[24%] min-w-[250px] ">
      {/* Search Bar */}
      <div className="p-2 bg-gray-200 dark:bg-black dark:border dark:border-white  rounded-full flex items-center  text-gray-500">
        <CiSearch size={"20px"} />
        <input
          type="text"
      ...
      {/* Who to Follow */}
      <div className="p-3 bg-gray-200 dark:bg-black dark:border rounded-2xl my-2 w-full">
        <h1 className="font-bold">Who To Follow</h1>
        <div className=" flex-col w-full mt-4 h-auto">
          {searchUsers?.map((user) => (
            <div
              key={user?._id}
              className="flex flex-row items-center justify-between w-full p-2 hover:bg-gray-100 dark:text-white rounded-lg"
            >
              {/* Avatar + Name */}
